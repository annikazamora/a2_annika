...
// 1) drawSplash() → what the splash/menu screen looks like
...
// ------------------------------------------------------------
// drawSplash() is called from main.js only when:
...
// ------------------------------------------------------------
// Mouse input for the splash screen
// ------------------------------------------------------------
// Called from main.js only when currentScreen === "splash"
function splashMousePressed() {
  // Do nothing if video is still playing
  if (!videoFinished) return;

  // For input checks, we only need x,y,w,h (label is optional)
  const playBtn = { x: 472, y: 570, w: 350, h: 100 };
  const instBtn = { x: 872, y: 570, w: 350, h: 100 };
...
// That logic lives in splashMousePressed() above.
//
